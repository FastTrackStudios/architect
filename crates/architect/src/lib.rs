...
/// (and providing) its `Connection<C>`.
///
/// With vox, `C` is `vox::Caller` — every feature builds its typed
/// clients from the shared caller, so adding a feature adds zero
/// connections:
///
/// ```ignore
/// // app root
...
/// field), re-establishing the connection.
///
/// See [`use_connect_reactive`] for the dependency-tracking contract.
#[cfg(feature = "atom")]
...
/// [`use_app_reactive`] with **supervision**.
///
/// In addition to the reactive connect contract, the established
/// connection is watched for death: when `watch_death(c)` resolves, the
/// connection flips back to `Connecting` and reconnects under a
/// [`Backoff`] (exponential, full jitter, floor 250ms, cap 10s, reset on
/// success), forever.
///
/// The [`Connection::generation`](architect_atom::Connection::generation)
...
/// [`use_app_supervised`] over the shared connection [`Pool`].
///
/// The app-root one-liner: resolve an [`Endpoint`] (reactively — an org
/// switcher, a server-URL field, a token that arrives after sign-in),
/// take the pooled connection for it, and supervise it. Every feature's
/// typed client is then a cheap view over the same caller, so adding a
/// feature adds zero sockets.
///
/// ```ignore
/// // app root
/// let conn = use_app_pooled(move || {
///     Endpoint::url(vox_url_for(&active_org())).with_bearer(session_token())
/// });
/// ```
///
/// Death is watched through `Caller::closed()`, and reconnection runs
/// under [`Backoff`] — see [`use_app_supervised`] for the full contract.
/// The pool is evicted for that endpoint first, so a reconnect always
/// dials rather than handing back the corpse.
#[cfg(all(feature = "atom", feature = "connect", feature = "platform"))]
pub fn use_app_pooled<F>(endpoint: F) -> Connection<vox_core::Caller>
where
    F: Fn() -> Endpoint + 'static,
{
    use_app_supervised(
        move || {
            let endpoint = endpoint();
            async move {
                // Evict first: a supervised reconnect runs precisely
                // because the cached root died, and `live()` would have
                // to notice that on its own. Being explicit here means a
                // reconnect can never serve the dead entry.
                pool().evict(&endpoint);
                pool().caller(&endpoint).await.map_err(|e| e.to_string())
            }
        },
...
pub fn use_interval(period: Duration) -> architect_atom::dioxus::prelude::Signal<u64> {
    ...
    use_future(move || async move {
        loop {
            ...
            // Saturating: at 1 tick/ms this needs ~584 million years to
            // reach `u64::MAX`, but the loop is infinite by construction.
...
/// wasm32.
///
/// Used for `impl Future` return/argument bounds the derive emits (a marker
/// trait is legal in impl-Trait bound position, unlike in a `dyn`
...
// One connection per (endpoint, identity), single-flighted and
// liveness-checked; typed clients are cheap views over its caller.
#[cfg(all(feature = "connect", feature = "vox"))]
pub mod connect;
#[cfg(all(feature = "connect", feature = "vox"))]
pub use connect::{ConnectError, Endpoint, Pool, pool};

// Poison-recovering lock accessors. Architect runs inside other people's
// processes — a REAPER extension, an audio host, a long-lived server — so
// a poisoned `Mutex` must never become a *second* panic at an unrelated
// call site. Public because every architect consumer has the same
// problem: `crdt`, the rig backends, and the fleet's own hubs.
pub mod lock;
pub use lock::{lock, read, write};

// Deterministic jitter — one splitmix64 shared by `schedule` and
// `reconnect`, which each carried an identical copy. Gated on the same
// features as its two consumers so it is not dead code without them.
#[cfg(any(feature = "platform", feature = "schedule"))]
mod jitter;
...
// `platform::Clock`.
//
// On with `platform`, not behind its own feature: `reconnect::Backoff` is
// a `Schedule` preset, so it is not optional relative to it — and "opt in
// to get retry policies" was a switch nothing in the fleet ever flipped
// while a dozen files hand-rolled their own backoff instead. `retry`,
// `repeat` and `Schedule` are re-exported at the crate root so the answer
// to "how do I retry this?" is one name away.
#[cfg(feature = "platform")]
pub mod schedule;
// Instance-scoped service mounting. The outer doc is a plain comment:
// an outer `///` here is concatenated with the module's own `//!` docs,
// which merges two summary sentences into one over-long paragraph.
#[cfg(feature = "vox")]
pub mod scoped;
#[cfg(feature = "platform")]
...
///
/// The **typed** app error is kept distinct from infrastructure failures,
/// so a page can `match` on `App(RepoError::NotFound)` while rendering
/// connectivity problems as a shared "reconnecting…" treatment.
///
...
impl<E> ClientError<E> {
    ...
    pub const fn app(&self) -> Option<&E> {
        match self {
            Self::App(e) => Some(e),
            _ => None,
        }
...
pub mod seed {
    ...
    impl SeedCount for u32 {
        fn pick(&self) -> usize {
            // A seed count is a row count. `try_into` can only fail on a
            // 16-bit target, where saturating is the sane answer anyway.
    ...
    impl SeedCount for i32 {
        fn pick(&self) -> usize {
            usize::try_from(self.max(&0).to_owned()).unwrap_or(usize::MAX)
        }
    }
    ...
    impl SeedCount for Range<u32> {
        fn pick(&self) -> usize {
            (self.start.pick()..self.end.pick()).pick()
        }
    }
...
pub mod dispatch {
    ...
    ///
    /// These wrap "transport-level" failures around the user's closure —
    /// the closure shutdown, the runtime tear-down, a panic inside the
    /// worker. Application-level errors returned from the closure flow
    /// through the future's `Output` unchanged.
    #[derive(Debug, Clone, PartialEq, Eq, facet::Facet, thiserror::Error)]
    #[repr(u8)]
    pub enum DispatchError {
        ...
        Cancelled,
        /// The dispatcher returned a value of a different type than the
        /// closure produced. A `Dispatcher` impl that does this is
        /// broken — but architect runs inside other people's processes,
        /// so a broken impl gets an error, not a panic in their thread.
        #[error("dispatcher returned a value of the wrong type")]
        TypeMismatch,
    }
    ...
    /// dispatcher boundary.
    ///
    /// Bridges downcast back to the concrete type they sent in; the
    /// type-erasure keeps `Dispatcher` object-safe without making it
    /// generic.
    #[cfg(not(target_arch = "wasm32"))]
    ...
    /// recover the typed result.
    ///
    /// This is the API bridge code generated by `#[architect::rpc]`
    /// actually calls — the trait method's `BoxedAny` shape stays a
    /// framework-internal detail.
    #[cfg(not(target_arch = "wasm32"))]
    ...
    where
        ...
        T: Send + 'static,
    {
        // `as BoxedAny` is an *unsizing coercion* to `Box<dyn Any>`, not a
        // numeric cast — nothing is truncated or reinterpreted.
    ...
    /// Calls the closure inline on the current thread.
    ///
    /// The returned future is always `Poll::Ready(Ok(_))` — no
    /// scheduling, no thread hop. Right for tests, single-threaded
    /// contexts, and in-process callers where there's no thread to
    /// marshal to.
    ...
    /// The build's default dispatcher.
    ///
    /// What `#[derive(architect::HasDispatcher)]` resolves to when no
    /// `#[dispatch(...)]` override is given: [`TokioBlockingDispatcher`]
    /// when the `dispatch-tokio` feature is enabled,
    /// [`CurrentThreadDispatcher`] otherwise.
    ///
    /// Server binaries that enable `dispatch-tokio` get the right
    /// default for free; tests and in-process callers get the inline
    /// dispatcher.
    ...
    /// `#[dispatch(...)]` override is given.
    ///
    /// Without `dispatch-tokio` this is the inline
    /// [`CurrentThreadDispatcher`].
    ...
    ///
    /// Right for server-side binaries that wrap sync-shaped backends (a
    /// sync DB driver, a CPU-bound transform) inside an async service.
    ...
    /// and awaits the result over a oneshot.
    ///
    /// This is the dispatcher for backends whose work must run under a
    /// specific thread affinity — a REAPER defer queue, a moiré
    /// main-thread executor, a GUI event loop — where the queue itself
    /// already exists and architect only needs a way to enqueue:
    /// ```ignore
...
pub mod action {
    ...
    //! queried.
    //!
    //! Distinct from `#[architect::rpc]`, which models data operations
    //! with arguments and return values. architect has zero awareness
    //! of any particular host here — no
    ...
    /// Owned counterpart to [`ActionMeta`].
    ///
    /// For a *family* of actions the `#[architect::actions]` macro can't
    /// express — a data-driven set whose ids/descriptions aren't known
    /// until runtime (e.g. one action per time signature, one per
    /// user-defined preset).
    ///
    /// [`Self::leak`] converts one into the `&'static ActionMeta`
    /// [`ActionBackend::register`] requires. Leaking is a deliberate,
    /// bounded choice here, not a bug:
    ...
    impl<B: ActionBackend> ScopedActionBackend<B> {
        pub const fn new(inner: B, scope_id: &'static str, scope_category: &'static str) -> Self {
            Self {
                inner,
                scope_id,
                scope_category,
